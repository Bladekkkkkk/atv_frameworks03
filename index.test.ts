...
test("Deve calcular a média corretamente quando há apenas homens", () => {
  const pessoas: Pessoa2[] = [
    { nome: "João", idade: 20, sexo: "M" },
    { nome: "Pedro", idade: 30, sexo: "M" }
  ];
  const mediaIdades = calcularMediaIdadePorSexo(pessoas);
  expect(mediaIdades).toEqual({ M: 25, F: NaN });
});
...
test("Deve calcular a média corretamente quando todas as idades são iguais", () => {
  const pessoas: Pessoa2[] = [
  ...
  const mediaIdades = calcularMediaIdadePorSexo(pessoas);
  expect(mediaIdades).toEqual({ M: 30, F: 30 });
});
...
function resolveEquacaoProvaA(vetor:number[]){
  let y:number[] = []
  for(let i=0;i<vetor.length;i++){
      let x = vetor[i]
      let calculo = (x**3 + 2*x**2 + 6*x + 5) / (x**2 + 2*x + 1) 
      y.push(calculo);
  }
  return y;
}
...
function resolveEquacaoProvaB(vetor:number[]){
  let y:number[] = []
  for(let i=0;i<vetor.length;i++){
      let x = vetor[i]
      let calculo = (x**3 + 2*x**2 + 6*x + 5) / (x**3 + 2*x**2 + 1) 
      y.push(calculo);
  }
  return y;
}
...
//Início do seu código
function encontrarLivrosPorCategoria(livros: LivroCategoria[],categoria:string) {
...
function aumentarPreco(items, factor){
  return items.map(item => ({...item,preco: item.preco * factor}));
}
...
test('Deve retornar um vetor com os novos preços corretos', () => {
  ...
  expect(novosPrecos).toEqual([
    ...
    { nome: "arroz", preco: 12.0 },
    { nome: "feijão", preco: 9.6 },
    { nome: "macarrão", preco: 6.0 }
...
test('aumentarPreco não altera o vetor de itens original', () => {
  ...
  const fator = 1.2;
  const resultado = aumentarPreco(compras, fator);
